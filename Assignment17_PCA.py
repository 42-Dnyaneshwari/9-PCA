# -*- coding: utf-8 -*-
"""
Created on Sat Nov 18 22:20:14 2023
...
df=pd.read_csv("C:/Datasets/Heart_Disease.csv")
df
df.describe()
df.columns
df.dtypes
...
df.columns
dic={   'Feature_Name':['sex', 'age', 'Qual_miles', 'cc1_miles', 'cc2_miles', 'cc3_miles',
       'cp', 'Bonus_trans', 'Flight_miles_12mo', 'Flight_trans_12',
...
lower_limit=df.age.quantile(0.75) - 1.5*IQR
lower_limit
#-18410.75
...
df_replaced=pd.DataFrame(np.where(df.age > upper_limit , upper_limit,np.where(df.age < lower_limit , lower_limit,df.age)))
...
# the given dataset is a imaged dataset

###################################################################
#scatter plot
df.plot(kind='scatter', x='cp', y='age') ;
plt.show()
#2D scatter plot
sns.set_style("whitegrid");
sns.FacetGrid(df, hue="sex").map(plt.scatter, "cp", "age").add_legend();
plt.show();
#pair plot
...
new_df=pd.read_csv("C:/Datasets/Heart_Disease.csv")
new_df.dtypes
new_df.describe()
new_df.columns
...
new_df
new_df.iloc[:,2:8].groupby(new_df.cp).mean()
...
import pandas as pd
import numpy as np
uni=pd.read_excel("C:/Datasets/Heart_Disease.csv")
uni
uni=pd.read_csv("C:/Datasets/Heart_Disease.csv")
uni.dtypes
