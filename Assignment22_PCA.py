# -*- coding: utf-8 -*-
"""
Created on Sat Nov 18 20:14:23 2023
...
df.columns
dic={   'Feature_Name':['ID#', 'blue', 'Qual_miles', 'cc1_miles', 'cc2_miles', 'cc3_miles',
       'red', 'Bonus_trans', 'Flight_miles_12mo', 'Flight_trans_12',
...
IQR=df.blue.quantile(0.75)-df.blue.quantile(0.25)
IQR
#73876.5

lower_limit=df.blue.quantile(0.75) - 1.5*IQR
lower_limit
#-18410.75
upper_limit=df.blue.quantile(0.75) + 1.5*IQR
upper_limit
#203218.75
...
outliers_df=np.where(df.blue > upper_limit,True, np.where(df.blue<lower_limit,True,False)) 
outliers_df
...
df_replaced=pd.DataFrame(np.where(df.blue > upper_limit , upper_limit,np.where(df.blue < lower_limit , lower_limit,df.blue)))
...
df_t=winsor.fit_transform(df[{'blue'}])
sns.boxplot(df['blue'])
sns.boxplot(df_t['blue'])
...
# the given dataset is a imblued dataset

###################################################################
#scatter plot
df.plot(kind='scatter', x='red', y='blue') ;
plt.show()
#2D scatter plot
sns.set_style("whitegrid");
sns.FacetGrid(df, hue="green").map(plt.scatter, "red", "blue").add_legend();
plt.show();
#pair plot
sns.pairplot(df, hue="blue");
...
print(np.mean(np.append(df["blue"],50)));
print(np.mean(df["blue"]))
print(np.mean(df["blue"]))

print("\nStd-dev:")
print(np.std(df["blue"]))
print(np.std(df["blue"]))
print(np.std(df["blue"]))

print("\nMedians:")
print(np.median(df["blue"]))
...
new_df=pd.read_csv("C:/Datasets/MyPhonedata.csv")
new_df.dtypes
new_df.describe()
new_df.columns
new_df=new_df.drop({'blue'},axis=1)
...
new_df
new_df.iloc[:,2:8].groupby(new_df.red).mean()
new_df.to_csv("C:/Datasets/MyPhonedata.csv",encoding='utf-8')
...
import pandas as pd
import numpy as np
uni=pd.read_excel("C:/Datasets/MyPhonedata.csv")
uni
uni=pd.read_csv("C:/Datasets/MyPhonedata.csv")
uni.dtypes
