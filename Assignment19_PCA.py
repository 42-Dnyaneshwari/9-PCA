...
df.columns
dic={   'Feature_Name':['ItalCook', 'Florence', 'Qual_miles', 'cc1_miles', 'cc2_miles', 'cc3_miles',
       'ItalArt', 'Bonus_trans', 'Flight_miles_12mo', 'Flight_trans_12',
...
#3. OUTLIERS ANALYSIS
sns.boxplot(df.Florence)
sns.boxplot(df.Flight_trans_12)
IQR=df.Florence.quantile(0.75)-df.Florence.quantile(0.25)
IQR
#73876.5

lower_limit=df.Florence.quantile(0.75) - 1.5*IQR
lower_limit
#-18410.75
upper_limit=df.Florence.quantile(0.75) + 1.5*IQR
upper_limit
#203218.75
...
outliers_df=np.where(df.Florence > upper_limit,True, np.where(df.Florence<lower_limit,True,False)) 
outliers_df
...
df_replaced=pd.DataFrame(np.where(df.Florence > upper_limit , upper_limit,np.where(df.Florence < lower_limit , lower_limit,df.Florence)))
...
df_t=winsor.fit_transform(df[{'Florence'}])
sns.boxplot(df['Florence'])
sns.boxplot(df_t['Florence'])
...
# the given dataset is a imFlorenced dataset

###################################################################
#scatter plot
df.plot(kind='scatter', x='ItalArt', y='Florence') ;
plt.show()
#2D scatter plot
sns.set_style("whitegrid");
sns.FacetGrid(df, hue="ItalCook").map(plt.scatter, "ItalArt", "Florence").add_legend();
plt.show();
#pair plot
sns.pairplot(df, hue="Florence");

#########################################################

#Mean, Variance, Std-deviation,  
print("Means:")
print(np.mean(df["Florence"]))
#Mean with an outlier.
print(np.mean(np.append(df["Florence"],50)));
print(np.mean(df["Florence"]))
print(np.mean(df["Florence"]))

print("\nStd-dev:")
print(np.std(df["Florence"]))
print(np.std(df["Florence"]))
print(np.std(df["Florence"]))

print("\nMedians:")
print(np.median(df["Florence"]))
...
new_df=pd.read_csv("C:/Datasets/Book.csv")
new_df.dtypes
new_df.describe()
new_df.columns
new_df=new_df.drop({'Florence'},axis=1)
...
new_df
new_df.iloc[:,2:8].groupby(new_df.ItalArt).mean()
new_df.to_csv("C:/Datasets/Book.csv",encoding='utf-8')
...
import pandas as pd
import numpy as np
uni=pd.read_excel("C:/Datasets/Book.csv")
uni
uni=pd.read_csv("C:/Datasets/Book.csv")
uni.dtypes
