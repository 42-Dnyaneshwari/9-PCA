# -*- coding: utf-8 -*-
"""
Created on Sat Nov 18 20:13:15 2023
...
df.columns
dic={   'Feature_Name':['Color', 'Ash', 'Qual_miles', 'cc1_miles', 'cc2_miles', 'cc3_miles',
       'Hue', 'Bonus_trans', 'Flight_miles_12mo', 'Flight_trans_12',
...
lower_limit=df.Ash.quantile(0.75) - 1.5*IQR
lower_limit
#-18410.75
...
outliers_df=np.where(df.Ash > upper_limit,True, np.where(df.Ash<lower_limit,True,False)) 
outliers_df
...
df_replaced=pd.DataFrame(np.where(df.Ash > upper_limit , upper_limit,np.where(df.Ash < lower_limit , lower_limit,df.Ash)))
...
df_t=winsor.fit_transform(df[{'Ash'}])
sns.boxplot(df['Ash'])
sns.boxplot(df_t['Ash'])
...
# the given dataset is a imAshd dataset

###################################################################
#scatter plot
df.plot(kind='scatter', x='Hue', y='Ash') ;
plt.show()
#2D scatter plot
sns.set_style("whitegrid");
sns.FacetGrid(df, hue="Color").map(plt.scatter, "Hue", "Ash").add_legend();
plt.show();
#pair plot
sns.pairplot(df, hue="Ash");

#########################################################

#Mean, Variance, Std-deviation,  
print("Means:")
print(np.mean(df["Ash"]))
#Mean with an outlier.
print(np.mean(np.append(df["Ash"],50)));
print(np.mean(df["Ash"]))
print(np.mean(df["Ash"]))
...
new_df=pd.read_csv("C:/Datasets/Wine.csv")
new_df.dtypes
new_df.describe()
new_df.columns
new_df=new_df.drop({'Ash'},axis=1)
...
new_df
new_df.iloc[:,2:8].groupby(new_df.Hue).mean()
new_df.to_csv("C:/Datasets/Wine.csv",encoding='utf-8')
...
import pandas as pd
import numpy as np
uni=pd.read_excel("C:/Datasets/Wine.csv")
uni
uni=pd.read_csv("C:/Datasets/Wine.csv")
uni.dtypes
