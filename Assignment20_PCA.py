# -*- coding: utf-8 -*-
"""
Created on Sat Nov 18 20:14:20 2023
...
df=pd.read_csv("C:/Datasets/Groceries.csv")
df
df.describe()
df.columns
df.dtypes
...
df.columns
dic={   'Feature_Name':['CookBks', 'ItalArt', 'Qual_miles', 'cc1_miles', 'cc2_miles', 'cc3_miles',
       'ItalCook', 'Bonus_trans', 'Flight_miles_12mo', 'Flight_trans_12',
...
#3. OUTLIERS ANALYSIS
sns.boxplot(df.ItalArt)
sns.boxplot(df.Flight_trans_12)
IQR=df.ItalArt.quantile(0.75)-df.ItalArt.quantile(0.25)
IQR
#73876.5

lower_limit=df.ItalArt.quantile(0.75) - 1.5*IQR
lower_limit
#-18410.75
upper_limit=df.ItalArt.quantile(0.75) + 1.5*IQR
upper_limit
#203218.75
...
outliers_df=np.where(df.ItalArt > upper_limit,True, np.where(df.ItalArt<lower_limit,True,False)) 
outliers_df
...
df_replaced=pd.DataFrame(np.where(df.ItalArt > upper_limit , upper_limit,np.where(df.ItalArt < lower_limit , lower_limit,df.ItalArt)))
...
df_t=winsor.fit_transform(df[{'ItalArt'}])
sns.boxplot(df['ItalArt'])
sns.boxplot(df_t['ItalArt'])
...
# the given dataset is a imItalArtd dataset

###################################################################
#scatter plot
df.plot(kind='scatter', x='ItalCook', y='ItalArt') ;
plt.show()
#2D scatter plot
sns.set_style("whitegrid");
sns.FacetGrid(df, hue="CookBks").map(plt.scatter, "ItalCook", "ItalArt").add_legend();
plt.show();
#pair plot
sns.pairplot(df, hue="ItalArt");

#########################################################

#Mean, Variance, Std-deviation,  
print("Means:")
print(np.mean(df["ItalArt"]))
#Mean with an outlier.
print(np.mean(np.append(df["ItalArt"],50)));
print(np.mean(df["ItalArt"]))
print(np.mean(df["ItalArt"]))

print("\nStd-dev:")
print(np.std(df["ItalArt"]))
print(np.std(df["ItalArt"]))
print(np.std(df["ItalArt"]))

print("\nMedians:")
print(np.median(df["ItalArt"]))
...
new_df=pd.read_csv("C:/Datasets/Groceries.csv")
new_df.dtypes
new_df.describe()
new_df.columns
new_df=new_df.drop({'ItalArt'},axis=1)
...
new_df
new_df.iloc[:,2:8].groupby(new_df.ItalCook).mean()
new_df.to_csv("C:/Datasets/Heart_Disease.csv",encoding='utf-8')
...
import pandas as pd
import numpy as np
uni=pd.read_excel("C:/Datasets/Groceries.csv")
uni
uni=pd.read_csv("C:/Datasets/Groceries.csv")
uni.dtypes
