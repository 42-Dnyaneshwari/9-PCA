# -*- coding: utf-8 -*-
"""
Created on Sat Nov 18 20:14:22 2023
...
df.columns
dic={   'Feature_Name':['LOTR2', 'Sixth Sense', 'Qual_miles', 'cc1_miles', 'cc2_miles', 'cc3_miles',
       'LOTR1', 'Bonus_trans', 'Flight_miles_12mo', 'Flight_trans_12',
...
#3. OUTLIERS ANALYSIS
sns.boxplot(df.Sixth Sense)
sns.boxplot(df.Flight_trans_12)
IQR=df.Sixth Sense.quantile(0.75)-df.Sixth Sense.quantile(0.25)
IQR
#73876.5

lower_limit=df.Sixth Sense.quantile(0.75) - 1.5*IQR
lower_limit
#-18410.75
upper_limit=df.Sixth Sense.quantile(0.75) + 1.5*IQR
upper_limit
#203218.75
...
outliers_df=np.where(df.Sixth Sense > upper_limit,True, np.where(df.Sixth Sense<lower_limit,True,False)) 
outliers_df
...
df_replaced=pd.DataFrame(np.where(df.Sixth Sense > upper_limit , upper_limit,np.where(df.Sixth Sense < lower_limit , lower_limit,df.Sixth Sense)))
...
winsor=Winsorizer(capping_method='iqr',
                  tail='both',
                  fold=1.5,
                  variables=['Sixth Sense'])

df_t=winsor.fit_transform(df[{'Sixth Sense'}])
sns.boxplot(df['Sixth Sense'])
sns.boxplot(df_t['Sixth Sense'])
...
# the given dataset is a imSixth Sensed dataset

###################################################################
#scatter plot
df.plot(kind='scatter', x='LOTR1', y='Sixth Sense') ;
plt.show()
#2D scatter plot
sns.set_style("whitegrid");
sns.FacetGrid(df, hue="LOTR2").map(plt.scatter, "LOTR1", "Sixth Sense").add_legend();
plt.show();
#pair plot
sns.pairplot(df, hue="Sixth Sense");

#########################################################

#Mean, Variance, Std-deviation,  
print("Means:")
print(np.mean(df["Sixth Sense"]))
#Mean with an outlier.
print(np.mean(np.append(df["Sixth Sense"],50)));
print(np.mean(df["Sixth Sense"]))
print(np.mean(df["Sixth Sense"]))

print("\nStd-dev:")
print(np.std(df["Sixth Sense"]))
print(np.std(df["Sixth Sense"]))
print(np.std(df["Sixth Sense"]))

print("\nMedians:")
print(np.median(df["Sixth Sense"]))
...
new_df=pd.read_csv("C:/Datasets/My_Movies.csv")
new_df.dtypes
new_df.describe()
new_df.columns
new_df=new_df.drop({'Sixth Sense'},axis=1)
...
new_df
new_df.iloc[:,2:8].groupby(new_df.LOTR1).mean()
new_df.to_csv("C:/Datasets/My_Movies.csv",encoding='utf-8')
...
import pandas as pd
import numpy as np
uni=pd.read_excel("C:/Datasets/My_Movies.csv")
uni
uni=pd.read_csv("C:/Datasets/My_Movies.csv")
uni.dtypes
